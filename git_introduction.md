...
Git est un système de version control qui enregistre les changements réalisés dans le cadre de vos projets. Bien que principalement destiné aux fichiers textes, git permet de placer n'importe quel type de fichier informatique dans une système de version control.
...
- git vous permet facilement de créer des branches distinctes au sein de vos projets, de les gérer en parallèle ou des les combiner en une seule version.
...
A ce stade, aucun fichier n'est encore monitoré par le système. Nous alons utiliser les commande `git add` pour ajouter les fichiers au stage et ensuite `git commit` pour enregistrer nos changements accompagnés d'un message.
...
`git commit -a -m "commit message"` permet d'ajouter vos fichier au stage et de  faire un commit en lui apposant le message "commit message"
...
Il est possible de revenir en arrière dans votre historique. C'est à cela que servent les commits.
